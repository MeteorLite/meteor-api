public class Player extends Actor {
    ...
    public Model getHeadModel() {
        ...
        for(int index = 0; index < 5; index++)
            if(appearanceColors[index] != 0) {
                headModel.replaceColor(Client.playerColours[index][0], Client.playerColours[index][appearanceColors[index]]);
                if(index == 1)
                    headModel.replaceColor(Client.SKIN_COLOURS[0], Client.SKIN_COLOURS[appearanceColors[index]]);
            }
        ...
        if(model == null) {
            ...
            for(int part = 0; part < 5; part++)
                if(appearanceColors[part] != 0) {
                    model.replaceColor(Client.playerColours[part][0], Client.playerColours[part][appearanceColors[part]]);
                    if(part == 1)
                        model.replaceColor(Client.SKIN_COLOURS[0], Client.SKIN_COLOURS[appearanceColors[part]]);
                }
    ...
    public Model getRotatedModel() {
        ...
        if(playerModel != null) {
            if(Client.pulseCycle >= objectAppearanceEndTick)
                playerModel = null;
            if(Client.pulseCycle >= objectAppearanceStartTick && Client.pulseCycle < objectAppearanceEndTick) {
    ...
    public void updateAppearance(Buffer buffer) {
        ...
        for(int l = 0; l < 5; l++) {
            ...
            if(j1 < 0 || j1 >= Client.playerColours[l].length)
                j1 = 0;
