...
public class Class22 {
    ...
    public boolean method249(int i) {
        ...
        for (int k : anIntArray457)
            if (!Model.method504(k))
                flag = false;

        return flag;
    }

    public Model method250(byte byte0) {
        if (byte0 != aByte452)
            aBoolean453 = !aBoolean453;
        if (anIntArray457 == null)
            return null;
        Model[] aclass44_sub3_sub4_sub4 = new Model[anIntArray457.length];
        for (int i = 0; i < anIntArray457.length; i++)
            aclass44_sub3_sub4_sub4[i] = Model.method503(anIntArray457[i], 6);
        ...
        for (int j = 0; j < 6; j++) {
            if (anIntArray458[j] == 0)
                break;
            model.method517(anIntArray458[j], anIntArray459[j]);
        }
    ...
    public boolean method251(int i) {
        boolean flag = true;
        for (int j = 0; j < 5; j++)
            if (anIntArray460[j] != -1 && !Model.method504(anIntArray460[j]))
                flag = false;
    ...
    public Model method252(byte byte0) {
        ...
        for (int j = 0; j < 5; j++)
            if (anIntArray460[j] != -1)
                aclass44_sub3_sub4_sub4[i++] = Model.method503(anIntArray460[j], 6);

        Model model = new Model(i, -643, aclass44_sub3_sub4_sub4);
        for (int k = 0; k < 6; k++) {
            if (anIntArray458[k] == 0)
                break;
            model.method517(anIntArray458[k], anIntArray459[k]);
        }
