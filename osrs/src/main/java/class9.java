...
public class class9 implements MouseWheel {
	...
	@Export("menuAction")
	static final void menuAction(int param0, int param1, int opcode, int id, String var4, String var5, int var6, int var7) {
		if (opcode >= 2000) { // L: 8803
			opcode -= 2000;
		}
		...
		if (opcode == 1) { // L: 8804
			...
			Client.destinationX = param0; // L: 9127
			Client.destinationY = param1; // L: 9128
			var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.field2994, Client.packetWriter.isaacCipher); // L: 9130
			var8.packetBuffer.writeShortLE(class432.selectedItemSlot); // L: 9131
			var8.packetBuffer.writeShortLE(Varcs.selectedItemId); // L: 9132
			var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9133
			var8.packetBuffer.writeShort(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9134
			var8.packetBuffer.writeShort(id); // L: 9135
			var8.packetBuffer.method7723(TaskHandler.selectedItemWidget); // L: 9136
			var8.packetBuffer.writeShortAdd(VarcInt.baseY + param1); // L: 9137
			...
			var8.packetBuffer.writeShortAdd(Client.selectedSpellItemId); // L: 9060
			var8.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9061
			var8.packetBuffer.writeShortAddLE(VarcInt.baseY + param1); // L: 9062
			var8.packetBuffer.writeShortAdd(Client.selectedSpellChildIndex); // L: 9063
			var8.packetBuffer.writeShortAdd(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9064
			var8.packetBuffer.method7723(Timer.selectedSpellWidget); // L: 9065
			var8.packetBuffer.writeShortLE(id); // L: 9066
			...
			Client.destinationX = param0; // L: 9577
			Client.destinationY = param1; // L: 9578
			var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPLOC1, Client.packetWriter.isaacCipher); // L: 9580
			var8.packetBuffer.writeShortAdd(VarcInt.baseY + param1); // L: 9581
			var8.packetBuffer.writeShortLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9582
			var8.packetBuffer.writeShort(id); // L: 9583
			var8.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9584
			...
			Client.destinationX = param0; // L: 9193
			Client.destinationY = param1; // L: 9194
			var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPLOC2, Client.packetWriter.isaacCipher); // L: 9196
			var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9197
			var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 9198
			var8.packetBuffer.writeShortAddLE(id); // L: 9199
			var8.packetBuffer.writeShortLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9200
			...
			Client.destinationX = param0; // L: 8874
			Client.destinationY = param1; // L: 8875
			...
			var8.packetBuffer.writeShortAdd(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 8879
			var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 8880
			var8.packetBuffer.writeShortAdd(id); // L: 8881
			...
			Client.destinationX = param0; // L: 8848
			Client.destinationY = param1; // L: 8849
			var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPLOC4, Client.packetWriter.isaacCipher); // L: 8851
			var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8852
			var8.packetBuffer.writeShortAddLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 8853
			var8.packetBuffer.writeShortAdd(id); // L: 8854
			var8.packetBuffer.writeShortAdd(VarcInt.baseY + param1); // L: 8855
			Client.packetWriter.addNode(var8); // L: 8856
		} else {
			PacketBufferNode var9;
			NPC var13;
			if (opcode == 7) {
				var13 = Client.npcs[id]; // L: 9034
				if (var13 != null) { // L: 9035
					...
					Client.destinationX = param0; // L: 9040
					Client.destinationY = param1; // L: 9041
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPCU, Client.packetWriter.isaacCipher); // L: 9043
					var9.packetBuffer.writeShortLE(id); // L: 9044
					var9.packetBuffer.writeIntIME(TaskHandler.selectedItemWidget); // L: 9045
					var9.packetBuffer.writeShortAddLE(Varcs.selectedItemId); // L: 9046
					var9.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9047
					var9.packetBuffer.writeShortAdd(class432.selectedItemSlot); // L: 9048
					Client.packetWriter.addNode(var9); // L: 9049
				}
			} else if (opcode == 8) {
				var13 = Client.npcs[id]; // L: 9165
				if (var13 != null) { // L: 9166
					...
					Client.destinationX = param0; // L: 9171
					Client.destinationY = param1; // L: 9172
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPCT, Client.packetWriter.isaacCipher); // L: 9173
					var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9174
					var9.packetBuffer.writeShort(Client.selectedSpellChildIndex); // L: 9175
					var9.packetBuffer.writeShortLE(id); // L: 9176
					var9.packetBuffer.writeIntLE(Timer.selectedSpellWidget); // L: 9177
					var9.packetBuffer.writeShortAdd(Client.selectedSpellItemId); // L: 9178
					Client.packetWriter.addNode(var9); // L: 9179
				}
			} else if (opcode == 9) {
				var13 = Client.npcs[id]; // L: 9332
				if (var13 != null) { // L: 9333
					...
					Client.destinationX = param0; // L: 9338
					Client.destinationY = param1; // L: 9339
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPC1, Client.packetWriter.isaacCipher); // L: 9341
					var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9342
					var9.packetBuffer.writeShortLE(id); // L: 9343
					Client.packetWriter.addNode(var9); // L: 9344
				}
			} else if (opcode == 10) {
				var13 = Client.npcs[id]; // L: 8950
				...
				if (var13 != null) { // L: 9071
					...
					Client.destinationX = param0; // L: 9076
					Client.destinationY = param1; // L: 9077
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPC3, Client.packetWriter.isaacCipher); // L: 9079
					var9.packetBuffer.writeShortLE(id); // L: 9080
					var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9081
					Client.packetWriter.addNode(var9); // L: 9082
				}
			} else if (opcode == 12) {
				var13 = Client.npcs[id]; // L: 8991
				if (var13 != null) { // L: 8992
					...
					Client.destinationX = param0; // L: 8997
					Client.destinationY = param1; // L: 8998
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPC4, Client.packetWriter.isaacCipher); // L: 9000
					var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9001
					var9.packetBuffer.writeShortLE(id); // L: 9002
					Client.packetWriter.addNode(var9); // L: 9003
				}
			} else if (opcode == 13) {
				var13 = Client.npcs[id]; // L: 9494
				if (var13 != null) { // L: 9495
					...
					Client.destinationX = param0; // L: 9500
					Client.destinationY = param1; // L: 9501
					var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPC5, Client.packetWriter.isaacCipher); // L: 9503
					var9.packetBuffer.writeShortLE(id); // L: 9504
					var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9505
					Client.packetWriter.addNode(var9); // L: 9506
				}
			} else {
				Player var15;
				if (opcode == 14) {
					var15 = Client.players[id]; // L: 9442
					if (var15 != null) { // L: 9443
						...
						Client.destinationX = param0; // L: 9448
						Client.destinationY = param1; // L: 9449
						var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYERU, Client.packetWriter.isaacCipher); // L: 9451
						var9.packetBuffer.writeShortAddLE(Varcs.selectedItemId); // L: 9452
						var9.packetBuffer.writeShort(id); // L: 9453
						var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9454
						var9.packetBuffer.writeShortAdd(class432.selectedItemSlot); // L: 9455
						var9.packetBuffer.writeIntIME(TaskHandler.selectedItemWidget); // L: 9456
						Client.packetWriter.addNode(var9); // L: 9457
					}
				} else if (opcode == 15) {
					var15 = Client.players[id]; // L: 9541
					if (var15 != null) { // L: 9542
						...
						Client.destinationX = param0; // L: 9547
						Client.destinationY = param1; // L: 9548
						var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYERT, Client.packetWriter.isaacCipher); // L: 9549
						var9.packetBuffer.writeShortLE(Client.selectedSpellChildIndex); // L: 9550
						var9.packetBuffer.writeShortAdd(id); // L: 9551
						var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9552
					...
					Client.destinationX = param0; // L: 8809
					Client.destinationY = param1; // L: 8810
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJU, Client.packetWriter.isaacCipher); // L: 8812
					var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8813
					var8.packetBuffer.writeShortAdd(class432.selectedItemSlot); // L: 8814
					var8.packetBuffer.writeShortAdd(id); // L: 8815
					var8.packetBuffer.writeShortAdd(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 8816
					var8.packetBuffer.writeShortAddLE(VarcInt.baseY + param1); // L: 8817
					var8.packetBuffer.writeIntIME(TaskHandler.selectedItemWidget); // L: 8818
					...
					Client.destinationX = param0; // L: 8889
					Client.destinationY = param1; // L: 8890
					...
					var8.packetBuffer.writeShortLE(id); // L: 8894
					var8.packetBuffer.writeInt(Timer.selectedSpellWidget); // L: 8895
					var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 8896
					var8.packetBuffer.writeShort(Client.selectedSpellChildIndex); // L: 8897
					var8.packetBuffer.writeShortAdd(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 8898
					...
					Client.destinationX = param0; // L: 9352
					Client.destinationY = param1; // L: 9353
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ1, Client.packetWriter.isaacCipher); // L: 9355
					var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9356
					var8.packetBuffer.writeShortAddLE(id); // L: 9357
					var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 9358
					var8.packetBuffer.writeShortLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9359
					...
					Client.destinationX = param0; // L: 9383
					Client.destinationY = param1; // L: 9384
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ2, Client.packetWriter.isaacCipher); // L: 9386
					var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 9387
					var8.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9388
					var8.packetBuffer.writeShort(id); // L: 9389
					var8.packetBuffer.writeShortLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9390
					...
					Client.destinationX = param0; // L: 8923
					Client.destinationY = param1; // L: 8924
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ3, Client.packetWriter.isaacCipher); // L: 8926
					var8.packetBuffer.writeShortLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 8927
					var8.packetBuffer.writeShort(id); // L: 8928
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8929
					var8.packetBuffer.writeShortAdd(VarcInt.baseY + param1); // L: 8930
					...
					Client.destinationX = param0; // L: 9530
					Client.destinationY = param1; // L: 9531
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ4, Client.packetWriter.isaacCipher); // L: 9533
					var8.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9534
					var8.packetBuffer.writeShort(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9535
					var8.packetBuffer.writeShortAddLE(VarcInt.baseY + param1); // L: 9536
					var8.packetBuffer.writeShortAddLE(id); // L: 9537
					...
					Client.destinationX = param0; // L: 9233
					Client.destinationY = param1; // L: 9234
					var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ5, Client.packetWriter.isaacCipher); // L: 9236
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9237
					var8.packetBuffer.writeShortAddLE(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9238
					var8.packetBuffer.writeShortLE(VarcInt.baseY + param1); // L: 9239
					var8.packetBuffer.writeShort(id); // L: 9240
					Client.packetWriter.addNode(var8); // L: 9241
				} else if (opcode == 23) {
					if (Client.isMenuOpen) { // L: 8823
						ArchiveLoader.scene.setViewportWalking(); // L: 8824
					} else {
						ArchiveLoader.scene.menuOpen(class18.Client_plane, param0, param1, true); // L: 8827
					}
				} else {
					PacketBufferNode var10;
					Widget var16;
					if (opcode == 24) {
						var16 = class127.getWidget(param1); // L: 8966
						if (var16 != null) { // L: 8967
							...
							if (var11) { // L: 8970
								...
								var10.packetBuffer.writeInt(param1); // L: 8973
								Client.packetWriter.addNode(var10); // L: 8974
							}
						}
					} else {
						if (opcode == 25) {
							var16 = ItemComposition.getWidgetChild(param1, param0); // L: 8831
							if (var16 != null) { // L: 8832
								FriendSystem.Widget_runOnTargetLeave(); // L: 8833
								Renderable.selectSpell(param1, param0, WorldMapID.Widget_unpackTargetMask(class326.getWidgetFlags(var16)), var16.itemId); // L: 8834
						...
						if (opcode == 26) {
							...
							if (opcode == 28) {
								...
								var8.packetBuffer.writeInt(param1); // L: 9206
								Client.packetWriter.addNode(var8); // L: 9207
								var14 = class127.getWidget(param1); // L: 9208
								...
								var8.packetBuffer.writeInt(param1); // L: 9561
								Client.packetWriter.addNode(var8); // L: 9562
								var14 = class127.getWidget(param1); // L: 9563
								...
								if (Client.meslayerContinueWidget == null) { // L: 9394
									class268.resumePauseWidget(param1, param0); // L: 9395
									Client.meslayerContinueWidget = ItemComposition.getWidgetChild(param1, param0); // L: 9396
								...
								var8.packetBuffer.writeIntIME(TaskHandler.selectedItemWidget); // L: 9320
								var8.packetBuffer.writeShortLE(class432.selectedItemSlot); // L: 9321
								var8.packetBuffer.writeIntLE(param1); // L: 9322
								var8.packetBuffer.writeShort(id); // L: 9323
								var8.packetBuffer.writeShort(param0); // L: 9324
								var8.packetBuffer.writeShortLE(Varcs.selectedItemId); // L: 9325
								Client.packetWriter.addNode(var8); // L: 9326
								Client.field610 = 0; // L: 9327
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9328
								Client.field611 = param0; // L: 9329
							} else if (opcode == 32) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELDT, Client.packetWriter.isaacCipher); // L: 9153
								var8.packetBuffer.writeShortAdd(id); // L: 9154
								var8.packetBuffer.writeIntIME(Timer.selectedSpellWidget); // L: 9155
								var8.packetBuffer.writeShortAdd(param0); // L: 9156
								var8.packetBuffer.writeShort(Client.selectedSpellChildIndex); // L: 9157
								var8.packetBuffer.writeInt(param1); // L: 9158
								Client.packetWriter.addNode(var8); // L: 9159
								Client.field610 = 0; // L: 9160
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9161
								Client.field611 = param0; // L: 9162
							} else if (opcode == 33) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELD1, Client.packetWriter.isaacCipher); // L: 9260
								var8.packetBuffer.writeShort(id); // L: 9261
								var8.packetBuffer.writeShort(param0); // L: 9262
								var8.packetBuffer.writeIntLE(param1); // L: 9263
								Client.packetWriter.addNode(var8); // L: 9264
								Client.field610 = 0; // L: 9265
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9266
								Client.field611 = param0; // L: 9267
							} else if (opcode == 34) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELD2, Client.packetWriter.isaacCipher); // L: 9589
								var8.packetBuffer.writeIntLE(param1); // L: 9590
								var8.packetBuffer.writeShort(param0); // L: 9591
								var8.packetBuffer.writeShort(id); // L: 9592
								Client.packetWriter.addNode(var8); // L: 9593
								Client.field610 = 0; // L: 9594
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9595
								Client.field611 = param0; // L: 9596
							} else if (opcode == 35) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELD3, Client.packetWriter.isaacCipher); // L: 9462
								var8.packetBuffer.writeShort(id); // L: 9463
								var8.packetBuffer.writeShort(param0); // L: 9464
								var8.packetBuffer.writeIntLE(param1); // L: 9465
								Client.packetWriter.addNode(var8); // L: 9466
								Client.field610 = 0; // L: 9467
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9468
								Client.field611 = param0; // L: 9469
							} else if (opcode == 36) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELD4, Client.packetWriter.isaacCipher); // L: 9113
								var8.packetBuffer.writeShortLE(id); // L: 9114
								var8.packetBuffer.writeIntLE(param1); // L: 9115
								var8.packetBuffer.writeShortAddLE(param0); // L: 9116
								Client.packetWriter.addNode(var8); // L: 9117
								Client.field610 = 0; // L: 9118
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9119
								Client.field611 = param0; // L: 9120
							} else if (opcode == 37) {
								var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPHELD5, Client.packetWriter.isaacCipher); // L: 8860
								var8.packetBuffer.writeIntIME(param1); // L: 8861
								var8.packetBuffer.writeShort(id); // L: 8862
								var8.packetBuffer.writeShortAddLE(param0); // L: 8863
								Client.packetWriter.addNode(var8); // L: 8864
								Client.field610 = 0; // L: 8865
								GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 8866
								Client.field611 = param0; // L: 8867
							} else {
								if (opcode == 38) {
									FriendSystem.Widget_runOnTargetLeave(); // L: 8979
									var16 = class127.getWidget(param1); // L: 8980
									Client.isItemSelected = 1; // L: 8981
									class432.selectedItemSlot = param0; // L: 8982
									TaskHandler.selectedItemWidget = param1; // L: 8983
									Varcs.selectedItemId = id; // L: 8984
									class346.invalidateWidget(var16); // L: 8985
									Client.selectedItemName = UserComparator9.colorStartTag(16748608) + class19.ItemDefinition_get(id).name + UserComparator9.colorStartTag(16777215); // L: 8986
								...
								if (opcode == 39) {
									...
									var8.packetBuffer.writeInt(param1); // L: 9025
									var8.packetBuffer.writeShortAdd(param0); // L: 9026
									var8.packetBuffer.writeShortLE(id); // L: 9027
									Client.packetWriter.addNode(var8); // L: 9028
									Client.field610 = 0; // L: 9029
									GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9030
									Client.field611 = param0; // L: 9031
								} else if (opcode == 40) {
									var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.IF1_BUTTON2, Client.packetWriter.isaacCipher); // L: 9142
									var8.packetBuffer.writeIntIME(param1); // L: 9143
									var8.packetBuffer.writeShort(param0); // L: 9144
									var8.packetBuffer.writeShortAdd(id); // L: 9145
									Client.packetWriter.addNode(var8); // L: 9146
									Client.field610 = 0; // L: 9147
									GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9148
									Client.field611 = param0; // L: 9149
								} else if (opcode == 41) {
									var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.IF1_BUTTON3, Client.packetWriter.isaacCipher); // L: 9432
									var8.packetBuffer.writeShortAdd(id); // L: 9433
									var8.packetBuffer.writeIntLE(param1); // L: 9434
									var8.packetBuffer.writeShortAddLE(param0); // L: 9435
									Client.packetWriter.addNode(var8); // L: 9436
									Client.field610 = 0; // L: 9437
									GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9438
									Client.field611 = param0; // L: 9439
								} else if (opcode == 42) {
									var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.IF1_BUTTON4, Client.packetWriter.isaacCipher); // L: 9484
									var8.packetBuffer.writeShortAddLE(param0); // L: 9485
									var8.packetBuffer.writeInt(param1); // L: 9486
									var8.packetBuffer.writeShortLE(id); // L: 9487
									Client.packetWriter.addNode(var8); // L: 9488
									Client.field610 = 0; // L: 9489
									GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9490
									Client.field611 = param0; // L: 9491
								} else if (opcode == 43) {
									var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.IF1_BUTTON5, Client.packetWriter.isaacCipher); // L: 9473
									var8.packetBuffer.writeShortAddLE(param0); // L: 9474
									var8.packetBuffer.writeShortAddLE(id); // L: 9475
									var8.packetBuffer.writeInt(param1); // L: 9476
									Client.packetWriter.addNode(var8); // L: 9477
									Client.field610 = 0; // L: 9478
									GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9479
									Client.field611 = param0; // L: 9480
								} else if (opcode == 44) {
									var15 = Client.players[id]; // L: 9286
									if (var15 != null) { // L: 9287
										...
										Client.destinationX = param0; // L: 9292
										Client.destinationY = param1; // L: 9293
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER1, Client.packetWriter.isaacCipher); // L: 9295
										var9.packetBuffer.writeShortLE(id); // L: 9296
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9297
										Client.packetWriter.addNode(var9); // L: 9298
									}
								} else if (opcode == 45) {
									var15 = Client.players[id]; // L: 8934
									if (var15 != null) { // L: 8935
										...
										Client.destinationX = param0; // L: 8940
										Client.destinationY = param1; // L: 8941
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER2, Client.packetWriter.isaacCipher); // L: 8943
										var9.packetBuffer.writeShortLE(id); // L: 8944
										var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8945
										Client.packetWriter.addNode(var9); // L: 8946
									}
								} else if (opcode == 46) {
									var15 = Client.players[id]; // L: 9086
									if (var15 != null) { // L: 9087
										...
										Client.destinationX = param0; // L: 9092
										Client.destinationY = param1; // L: 9093
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER3, Client.packetWriter.isaacCipher); // L: 9095
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9096
										var9.packetBuffer.writeShortAdd(id); // L: 9097
										Client.packetWriter.addNode(var9); // L: 9098
									}
								} else if (opcode == 47) {
									var15 = Client.players[id]; // L: 9510
									if (var15 != null) { // L: 9511
										...
										Client.destinationX = param0; // L: 9516
										Client.destinationY = param1; // L: 9517
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER4, Client.packetWriter.isaacCipher); // L: 9519
										var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9520
										var9.packetBuffer.writeShort(id); // L: 9521
										Client.packetWriter.addNode(var9); // L: 9522
									}
								} else if (opcode == 48) {
									var15 = Client.players[id]; // L: 9415
									if (var15 != null) { // L: 9416
										...
										Client.destinationX = param0; // L: 9421
										Client.destinationY = param1; // L: 9422
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER5, Client.packetWriter.isaacCipher); // L: 9424
										var9.packetBuffer.writeShort(id); // L: 9425
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9426
										Client.packetWriter.addNode(var9); // L: 9427
									}
								} else if (opcode == 49) {
									var15 = Client.players[id]; // L: 9270
									...
									if (var15 != null) { // L: 9364
										...
										Client.destinationX = param0; // L: 9369
										Client.destinationY = param1; // L: 9370
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER7, Client.packetWriter.isaacCipher); // L: 9372
										var9.packetBuffer.writeShort(id); // L: 9373
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9374
										Client.packetWriter.addNode(var9); // L: 9375
									}
								} else if (opcode == 51) {
									var15 = Client.players[id]; // L: 9302
									if (var15 != null) { // L: 9303
										...
										Client.destinationX = param0; // L: 9308
										Client.destinationY = param1; // L: 9309
										var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPPLAYER8, Client.packetWriter.isaacCipher); // L: 9311
										var9.packetBuffer.writeShortAddLE(id); // L: 9312
										var9.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9313
										Client.packetWriter.addNode(var9); // L: 9314
									}
								} else if (opcode != 1007 && opcode != 57) {
									if (opcode == 58) {
										var16 = ItemComposition.getWidgetChild(param1, param0); // L: 9401
										if (var16 != null) { // L: 9402
											...
											var9.packetBuffer.writeIntIME(Timer.selectedSpellWidget); // L: 9406
											var9.packetBuffer.writeShortAdd(var16.itemId); // L: 9407
											var9.packetBuffer.writeShortAddLE(Client.selectedSpellChildIndex); // L: 9408
											var9.packetBuffer.writeInt(param1); // L: 9409
											var9.packetBuffer.writeShortAdd(param0); // L: 9410
										...
										Client.destinationX = param0; // L: 9248
										Client.destinationY = param1; // L: 9249
										var8 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPLOC5, Client.packetWriter.isaacCipher); // L: 9251
										var8.packetBuffer.writeShortLE(id); // L: 9252
										var8.packetBuffer.writeShortAdd(param0 + GrandExchangeOfferOwnWorldComparator.baseX); // L: 9253
										var8.packetBuffer.writeByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9254
										var8.packetBuffer.writeShort(VarcInt.baseY + param1); // L: 9255
										...
										var8.packetBuffer.writeShortAddLE(id); // L: 9108
										...
										if (var13 != null) { // L: 8907
											...
											if (var17 != null) { // L: 8910
												var10 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPNPC6, Client.packetWriter.isaacCipher); // L: 8912
												var10.packetBuffer.writeShortLE(var17.id); // L: 8913
										...
										var8.packetBuffer.writeShortAdd(id); // L: 9225
										Client.packetWriter.addNode(var8); // L: 9226
									} else if (opcode == 1005) {
										var16 = class127.getWidget(param1); // L: 9007
										if (var16 != null && var16.itemQuantities[param0] >= 100000) { // L: 9008
											Actor.addGameMessage(27, "", var16.itemQuantities[param0] + " x " + class19.ItemDefinition_get(id).name);
										} else {
											var9 = WorldMapSprite.getPacketBufferNode(ClientPacket.OPOBJ6, Client.packetWriter.isaacCipher); // L: 9011
											var9.packetBuffer.writeShortAdd(id); // L: 9012
											Client.packetWriter.addNode(var9); // L: 9013
										}

										Client.field610 = 0; // L: 9015
										GrandExchangeOfferAgeComparator.field4011 = class127.getWidget(param1); // L: 9016
										Client.field611 = param0; // L: 9017
									} else if (opcode == 1011 || opcode == 1010 || opcode == 1009 || opcode == 1008 || opcode == 1012) {
										ScriptFrame.worldMap.worldMapMenuAction(opcode, id, new Coord(param0), new Coord(param1)); // L: 9216
									}
								} else {
									var16 = ItemComposition.getWidgetChild(param1, param0); // L: 9183
									if (var16 != null) { // L: 9184
										WorldMapLabelSize.widgetDefaultMenuAction(id, param1, param0, var16.itemId, var5); // L: 9185
									}
								}
							}
						}
