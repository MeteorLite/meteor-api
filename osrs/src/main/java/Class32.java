...
public class Class32 {
    ...
    public Model method271() {
        Model model = (Model) aClass39_559.method339(anInt548);
        if (model != null)
            return model;
        model = Model.method503(anInt549, 6);
        if (model == null)
            return null;
        for (int i = 0; i < 6; i++)
            if (anIntArray552[0] != 0)
                model.method517(anIntArray552[i], anIntArray553[i]);

        aClass39_559.method340(201, anInt548, model);
        return model;
    }
