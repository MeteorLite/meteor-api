...
public class JagFont extends Drawable {
	...
	public void method481(byte abyte0[], int i, int j, int k, int l, int i1) {
		int j1 = i + j * Drawable.width;
		int k1 = Drawable.width - k;
		int l1 = 0;
		int i2 = 0;
		if (j < Drawable.anInt1427) {
			int j2 = Drawable.anInt1427 - j;
			l -= j2;
			j = Drawable.anInt1427;
			i2 += j2 * k;
			j1 += j2 * Drawable.width;
		}
		if (j + l >= Drawable.anInt1428)
			l -= ((j + l) - Drawable.anInt1428) + 1;
		if (i < Drawable.anInt1429) {
			int k2 = Drawable.anInt1429 - i;
			k -= k2;
		...
		if (i + k >= Drawable.anInt1430) {
			int l2 = ((i + k) - Drawable.anInt1430) + 1;
			k -= l2;
			l1 += l2;
			k1 += l2;
		}
		if (k <= 0 || l <= 0) {
			...
			method482(Drawable.anIntArray1424, abyte0, i1, i2, j1, k, l, k1, l1);
			return;
		}
	}
