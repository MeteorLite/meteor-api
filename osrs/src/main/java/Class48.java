...
public class Class48 {
  ...
  public static void method434(Archive class2, int i) {
    Buffer class50_sub1_sub2 = new Buffer(class2.extract("idk.dat"));
    anInt814 = class50_sub1_sub2.readUShort();
  ...
  public void method435(byte byte0, Buffer class50_sub1_sub2) {
    ...
    do {
      ...
      if (i == 1) {
        anInt816 = class50_sub1_sub2.readUByte();
      } else if (i == 2) {
        int j = class50_sub1_sub2.readUByte();
        anIntArray817 = new int[j];
        for (int k = 0; k < j; k++) {
          anIntArray817[k] = class50_sub1_sub2.readUShort();
        }

      } else if (i == 3) {
        aBoolean821 = true;
      } else if (i >= 40 && i < 50) {
        anIntArray818[i - 40] = class50_sub1_sub2.readUShort();
      } else if (i >= 50 && i < 60) {
        anIntArray819[i - 50] = class50_sub1_sub2.readUShort();
      } else if (i >= 60 && i < 70) {
        anIntArray820[i - 60] = class50_sub1_sub2.readUShort();
      } else {
  ...
  public Model method437(byte byte0) {
    ...
    for (int i = 0; i < anIntArray817.length; i++) {
      aclass50_sub1_sub4_sub4[i] = Model
          .method577(anIntArray817[i]);
    }
    ...
    for (int j = 0; j < 6; j++) {
      ...
      class50_sub1_sub4_sub4
          .method591(anIntArray818[j], anIntArray819[j]);
    }
  ...
  public boolean method438(int i) {
    ...
    for (int j = 0; j < 5; j++) {
      if (anIntArray820[j] != -1
          && !Model.method578(anIntArray820[j])) {
        flag = false;
      }
    }
  ...
  public Model method439(boolean flag) {
    ...
    for (int j = 0; j < 5; j++) {
      if (anIntArray820[j] != -1) {
        aclass50_sub1_sub4_sub4[i++] = Model
            .method577(anIntArray820[j]);
      }
    }

    Model class50_sub1_sub4_sub4 = new Model(
        i, aclass50_sub1_sub4_sub4, (byte) -89);
    for (int k = 0; k < 6; k++) {
      if (anIntArray818[k] == 0) {
        break;
      }
      class50_sub1_sub4_sub4
          .method591(anIntArray818[k], anIntArray819[k]);
    }
