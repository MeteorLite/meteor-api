...
public class Client extends Game {
    ...
    private ImageProducer aClass18_906;
    private ImageProducer aClass18_907;
    private ImageProducer aClass18_908;
    private ImageProducer aClass18_909;
    private ImageProducer aClass18_910;
    private ImageProducer aClass18_911;
    private ImageProducer aClass18_912;
    private ImageProducer aClass18_913;
    private ImageProducer aClass18_914;
    ...
    private ImageProducer aClass18_1108;
    private ImageProducer aClass18_1109;
    private ImageProducer aClass18_1110;
    ...
    private ImageProducer aClass18_1157;
    private ImageProducer gameScreenImageProducer;
    private ImageProducer chatboxProducingGraphicsBuffer;
    ...
    private ImageProducer aClass18_1198;
    private ImageProducer aClass18_1199;
    private ImageProducer aClass18_1200;
    private ImageProducer flameLeftBackground;
    private ImageProducer flameRightBackground;
    private ImageProducer aClass18_1203;
    private ImageProducer aClass18_1204;
    private ImageProducer aClass18_1205;
    private ImageProducer aClass18_1206;
...
//            getAppletContext().showDocument(new URL(getCodeBase(), "loaderror_" + s + ".html"));
    ...
    private void saveMidi(boolean flag, byte[] abyte0) {
        if (musicEnabled) {
            ...
            SignLink.playMidi(abyte0, abyte0.length);
        }
    }
    ...
    private boolean parseIncomingPacket() {
        ...
        try {
            ...
            if (CHATBOX_MESSAGE.equals(opcode)) { // server message
    ...
    private void resetTitleScreen() {
        ...
        flameLeftBackground = new ImageProducer(128, 265, getParentComponent());
        Rasterizer.resetPixels();

        flameRightBackground = new ImageProducer(128, 265, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1198 = new ImageProducer(509, 171, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1199 = new ImageProducer(360, 132, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1200 = new ImageProducer(360, 200, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1203 = new ImageProducer(202, 238, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1204 = new ImageProducer(203, 238, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1205 = new ImageProducer(74, 94, getParentComponent());
        Rasterizer.resetPixels();

        aClass18_1206 = new ImageProducer(75, 94, getParentComponent());
        Rasterizer.resetPixels();
...
//                }
            ...
            aClass18_906 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backleft2", 0);
            aClass18_907 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backright1", 0);
            aClass18_908 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backright2", 0);
            aClass18_909 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backtop1", 0);
            aClass18_910 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backvmid1", 0);
            aClass18_911 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backvmid2", 0);
            aClass18_912 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backvmid3", 0);
            aClass18_913 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);

            image = new ImageRGB(archiveMedia, "backhmid2", 0);
            aClass18_914 = new ImageProducer(image.width, image.height, getParentComponent());
            image.drawInverse(0, 0);
        ...
        do {
            ...
            do {
                ...
                if (onDemandNode.type == 2 && onDemandNode.id == nextSong && onDemandNode.buffer != null)
                    AudioPlayer.playSong(onDemandNode.buffer);
...
//    }
    ...
    private boolean playSound(int length, byte[] data) {
            return SignLink.playSound(data, length);
    }
    ...
    private void method122() {
        if (chatboxProducingGraphicsBuffer == null) {
            ...
            aClass18_1205 = null;
            aClass18_1206 = null;
            chatboxProducingGraphicsBuffer = new ImageProducer(479, 96, getParentComponent());
            aClass18_1157 = new ImageProducer(172, 156, getParentComponent());
            Rasterizer.resetPixels();
            minimapBackgroundImage.drawImage(0, 0);
            tabImageProducer = new ImageProducer(190, 261, getParentComponent());
            gameScreenImageProducer = new ImageProducer(512, 334, getParentComponent());
            Rasterizer.resetPixels();
            aClass18_1108 = new ImageProducer(496, 50, getParentComponent());
            aClass18_1109 = new ImageProducer(269, 37, getParentComponent());
            aClass18_1110 = new ImageProducer(249, 45, getParentComponent());
            welcomeScreenRaised = true;
            gameScreenImageProducer.createRasterizer();
            Rasterizer3D.lineOffsets = viewportOffsets;
        }
    ...
    private void parsePlacementPacket(Buffer buf, int opcode) {
        if (opcode == 203) { // ??? something with object spawning?
        ...
        if (opcode == 142) { // object animation???
    ...
    private void resetAllImageProducers() {
        ...
        aClass18_1109 = null;
        aClass18_1110 = null;
        super.imageProducer = new ImageProducer(765, 503, getParentComponent());
        welcomeScreenRaised = true;
    }
        ...
        for (int index = 0; index < currentSound; index++) {
            ...
            try {
                ...
                if (System.currentTimeMillis() + (long) (stream.currentPosition / 22) > lastSoundTime
                    ...
                    if (playSound(stream.currentPosition, stream.buffer)) {
