...
public abstract class ClientMixin implements RSClient {
  ...
  public static void gameStateChanged(int idx)
  {
    ...
    client.getCallbacks().post(Events.GAME_STATE_CHANGED, new eventbus.events.GameStateChanged(null, gameState));
        ...
        for (int y = 0; y < 104; y++)
        {
          ...
          if (itemDeque != null)
          {
            ...
            for (RSNode current = head.getNext(); current != head; current = current.getNext())
            {
              ...
              item.setX(x);
              item.setY(y);
              client.getCallbacks().post(Events.ITEM_SPAWNED, new eventbus.events.ItemSpawned(tile, item));
            }
          }
        }
      }
  ...
  public static void experiencedChanged(int idx) {
    ...
    if (idx < possibleSkills.length - 1) {
      ...
      client.getCallbacks().post(Events.STAT_CHANGED, new eventbus.events.StatChanged(
              updatedSkill,
              newXp,
              level,
  ...
  public static void experienceGained(int oldExp, int currentExp, int skillLevel, Skill updatedSkill) {
    if (currentExp > oldExp) {
      client.getCallbacks().post(Events.EXPERIENCE_GAINED, new eventbus.events.ExperienceGained(
              updatedSkill,
              oldExp,
              currentExp,
              skillLevel
  ...
  public static void onMenuOptionsChanged(int idx) {
    ...
    if (newCount == oldCount + 1) {MenuEntryAdded event = new MenuEntryAdded(
      ...
      client.getCallbacks().post(Events.MENU_ENTRY_ADDED, event);
  ...
  public static void onCycleCntrChanged(int idx) {
    client.getCallbacks().post(Events.CLIENT_TICK, eventbus.events.ClientTick.INSTANCE);
  }
  ...
  public static void copy$runWidgetOnLoadListener(int groupId)
  {
    ...
    if (loaded)
    {
      ...
      client.getCallbacks().post(Events.WIDGET_LOADED, new WidgetLoaded(groupId));
    }
  }
  ...
  public static void canvasWidthChanged(int idx)
  {
    ...
    client.getCallbacks().post(Events.CANVAS_SIZE_CHANGED, eventbus.events.CanvasSizeChanged.INSTANCE);
  }
  ...
  {
    client.getCallbacks().post(Events.VARBIT_CHANGED, new VarbitChanged(idx));
  }
